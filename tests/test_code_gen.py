...
def _mdiff():
    ...
    """
        '''
        # Raw docstrings can't exact-roundtrip (AST doesn't preserve r prefix),
        # but the AST structure should be equivalent.
    ...
    def test_fstring_trailing_newline_triple_quoted(self):
        # Triple-quoted f-strings with literal newlines normalize to
        # single-line with escape sequences (same as CPython's ast.unparse).
        # AST is preserved, but exact source form is not.
        source = '''
        x = f"""{host}\n\t{port}\n"""
        '''
        self.assertAstRoundtripsGtVer(source, (3, 6))

    def test_fstring_trailing_newline_escaped(self):
        source = '''
        if 1:
            x = f'{host}\\n\\t{port}\\n'
        '''
...
class StdlibRoundtripRegressionTestCase(unittest.TestCase, Comparisons):
    """Test cases derived from stdlib files that fail to round-trip.

    Each test targets a specific code pattern that causes astor's
    generated source to fail compilation or produce an incorrect AST.
    """

    maxDiff = None

    def assertGeneratedCompiles(self, srctxt):
        """Assert that source generated by astor can be compiled."""
        srctxt = canonical(srctxt)
        tree = ast.parse(srctxt)
        generated = astor.to_source(tree)
        compile(generated, '<test>', 'exec')

    def assertGeneratedCompilesFromTree(self, tree):
        """Assert that source generated from an AST can be compiled."""
        ast.fix_missing_locations(tree)
        generated = astor.to_source(tree)
        compile(generated, '<test>', 'exec')

    # -- F-string quote delimiter conflicts --
    # From: asyncio/__main__.py, test/test_posix.py, test/test_tabnanny.py,
    #       xml/dom/minidom.py
    # Bug: when f-string text contains both ' and ", astor picks a
    # single-char delimiter that conflicts with one of them.

    def test_fstring_both_quote_types(self):
        """F-string literal text containing both single and double quotes."""
        source = '''x = f"""it's "great" {v}"""'''
        self.assertGeneratedCompiles(source)

    def test_fstring_double_quote_in_text_with_expr(self):
        """F-string with " in text and a string-containing expression."""
        source = """x = f'"x{f("a")}'"""
        self.assertGeneratedCompiles(source)

    # -- F-string unescaped special characters --
    # From: http/client.py, test/test_xmlrpc.py

    def test_fstring_carriage_return(self):
        r"""F-string with \r\n: \r is not re-escaped in generated source."""
        source = r"x = f'{v}\r\n'"
        self.assertGeneratedCompiles(source)

    # From: multiprocessing/connection.py

    def test_fstring_null_byte(self):
        r"""F-string with \0: null byte is not re-escaped in generated source."""
        source = r'x = f"\0{v}"'
        self.assertGeneratedCompiles(source)

    # -- Docstring handling in non-function bodies --
    # From: ctypes/__init__.py
    # Bug: body() treats the first Expr(Constant(str)) in any compound
    # statement as a docstring, but this is only valid for
    # function/class/module bodies.

    def test_string_in_if_body(self):
        """String expression as first statement in if-body is not a docstring."""
        source = '''
        if x:
            """A"""
            y = 1
    ...
    # -- Docstring ending with double-quote --
    # From: test/test_gdb.py
    # Bug: when a docstring value ends with ", wrapping in """ produces
    # """...X"""" where the trailing " starts an unclosed string literal.

    def test_docstring_ending_with_double_quote(self):
        """Docstring whose value ends with a " character."""
        source = r'''
    ...
    # -- Positional-only parameter default assignment --
    # From: test/inspect_fodder.py
    # Bug: visit_arguments miscalculates default offset when posonlyargs
    # exist without defaults, shifting defaults onto wrong params.

    def test_posonly_params_with_regular_defaults(self):
        """Positional-only params without defaults + regular params with defaults."""
        source = 'def f(a, /, b, c=1, d=2): pass'
        self.assertAstRoundtrips(source)

    # -- F-string expression with conflicting quotes --
    # From: test/test_fstring.py
    # Bug: f-string expression containing a string with ' gets rendered
    # with " around it, but the f-string itself also uses " delimiter.

    def test_fstring_expr_with_apostrophe(self):
        """F-string expression containing a string with an apostrophe."""
        source = """f"{'''it's'''}" """
        self.assertGeneratedCompiles(source)
