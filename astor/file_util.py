...
class JsonToAst(object):
    """Load an AST form a JSON representation.

    The JSON format supported by this function is the one generated by `astexport`.

    See https://github.com/fpoli/python-astexport.
    """

    @classmethod
    def object_to_ast(cls, d):
        # Extract AST class and arguments
        klass = None
        args = {}
        ...
        if sys.version_info.major == 3:
            ...
            node_iter = d.iteritems

        for k, v in node_iter():
            if k == "ast_type":
                klass = getattr(ast, v)
            else:
                if isinstance(v, list):
                    arg = [cls.data_to_ast(v) for v in v]
                elif isinstance(v, dict):
                    arg = cls.data_to_ast(v)
                else:
                    arg = v
    ...
    @classmethod
    def data_to_ast(cls, data):
        if not isinstance(data, dict):
            return data

        ast_type = data["ast_type"]

        if ast_type in ("int", "float"):
            return data["n"]
        elif ast_type == "complex":
            return complex(data["n"], data["i"])
        else:
            return cls.object_to_ast(data)

    def __call__(self, json_data):
        return self.data_to_ast(json_data)


json_to_ast = JsonToAst()
